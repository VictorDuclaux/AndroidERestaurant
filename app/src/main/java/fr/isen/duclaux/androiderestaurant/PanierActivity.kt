...
import fr.isen.duclaux.androiderestaurant.databinding.ActivityPanierBinding
import org.json.JSONObject

private lateinit var binding: ActivityPanierBinding
private lateinit var basket: Panier

class PanierActivity : AppCompatActivity(), BasketCellInterface {
    ...
    private fun sendOrder(idUser: Int) {
        val message = basket.items.map { "${it.nb}x ${it.item.nameItem}" }.joinToString("\n")
    ...
    override fun onShowDetail(item: PanierItem) {
        val fragment = DetailViewFragment(item.item)
